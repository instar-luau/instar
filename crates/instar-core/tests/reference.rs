...
fn preserve_keeps_the_blank_at_both_edges() {
    ...
    assert_eq!(
        configured(
            source,
            gaps(instar_core::format::configuration::Gaps::Preserve)
        ),
        source
    );
}
...
fn preserve_keeps_one_edge_when_only_one_has_a_blank() {
    let configuration = gaps(instar_core::format::configuration::Gaps::Preserve);
...
fn const_converts_a_local_require() {
    assert_eq!(
        configured(
            ...
            binding(instar_core::format::configuration::Binding::Const)
        ),
...
fn local_converts_a_const_require_back() {
    assert_eq!(
        configured(
            "const Signal = require(\"@pkg/signal\")\nreturn Signal\n",
            binding(instar_core::format::configuration::Binding::Local)
        ),
        "local Signal = require(\"@pkg/signal\")\nreturn Signal\n"
    );
}
...
fn only_a_single_unannotated_binding_converts() {
    let configuration = binding(instar_core::format::configuration::Binding::Const);
...
fn semicolons(mode: instar_core::format::configuration::Semicolons) -> Options {
    Options {
        semicolons: mode,
        ..Default::default()
    }
...
fn always_terminates_every_statement() {
    assert_eq!(
        configured(
            "local a = 1\nlocal b = 2\nreturn b\n",
            semicolons(instar_core::format::configuration::Semicolons::Always)
        ),
        "local a = 1;\nlocal b = 2;\nreturn b;\n"
    );
}
...
fn the_one_semicolon_luau_requires_survives_every_setting() {
    for mode in [
        instar_core::format::configuration::Semicolons::Never,
        instar_core::format::configuration::Semicolons::Always,
    ] {
...
fn terminating_every_statement_is_idempotent() {
    let configuration = semicolons(instar_core::format::configuration::Semicolons::Always);
...
fn the_method_style_breaks_at_each_call() {
    use instar_core::format::configuration::Chain;
...
fn zero(mode: instar_core::format::configuration::Zero) -> Options {
    Options {
        leading_zero: mode,
        ..Default::default()
    }
...
fn strip_removes_the_leading_zero_where_a_fraction_follows() {
    use instar_core::format::configuration::Zero;
