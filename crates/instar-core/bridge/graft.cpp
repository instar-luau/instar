...
static void serialize(lua_State *state, int index, std::string &output, size_t limit,
                      std::unordered_set<const void *> &active, unsigned depth) {
    ...
    case LUA_TNUMBER: {
        ...
        if (!std::isfinite(value) || std::trunc(value) != value || value < 0 ||
            value >= static_cast<double>(std::numeric_limits<unsigned long long>::max())) {
            throw std::runtime_error("graft response numbers must be nonnegative integers");
        }

        append(output, std::to_string(static_cast<unsigned long long>(value)), limit);
    ...
    if (!array && !object) {
        throw std::runtime_error("graft response tables must be records or dense arrays");
    }
...
static void run(lua_State *state, Bytes source, const char *name) {
    ...
    if (luau_load(state, name, bytecode.data(), bytecode.size(), 0) != 0 || lua_pcall(state, 0, 1, 0) != 0) {
        const char *error = lua_tostring(state, -1);

        throw std::runtime_error(error ? error : "Luau graft raised a non-string error");
    }
}

extern "C" void instar_graft(Bytes source, Bytes request, bool format, bool lint, bool compile, size_t limit,
                             void *context, Reply reply) {
    try {
        instar_initialize();

        std::unique_ptr<lua_State, void (*)(lua_State *)> owner(luaL_newstate(), lua_close);
        ...
        for (const char *hook : {"format", "lint", "compile"}) {
            if ((std::string_view(hook) == "format" && !format) || (std::string_view(hook) == "lint" && !lint) ||
            ...
            if (!lua_isfunction(state, -1)) {
                throw std::runtime_error(std::string("Luau graft exports no ") + hook + " function");
            }
