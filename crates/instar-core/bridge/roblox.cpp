...
std::optional<Luau::TypeId> instance(const Luau::Scope &scope,
  ...
  return parent && type && Luau::isSubclass(type, parent)
             ? std::optional<Luau::TypeId>{target->type}
             : std::nullopt;
}

void report(const Luau::MagicFunctionCallContext &context,
            const Luau::Location &location, const std::string &message) {
  if (context.constraint->moduleName)
    context.solver->reportError(Luau::GenericError{message}, location,
                                *context.constraint->moduleName);
  else
    context.solver->DEPRECATED_reportError(
        Luau::TypeError{location, Luau::GenericError{message}});
}

struct Predicate : Luau::MagicFunction {
  std::optional<Luau::WithPredicate<Luau::TypePackId>>
  handleOldSolver(Luau::TypeChecker &checker, const Luau::ScopePtr &scope,
    ...
    if (!type)
      checker.reportError(Luau::TypeError{
          literal->location,
          Luau::GenericError{"Unknown instance class '" + name + "'"}});

    if (!value || !type)
      return std::nullopt;

    return Luau::WithPredicate<Luau::TypePackId>{
        checker.currentModule->internalTypes->addTypePack(
            {checker.booleanType}),
  ...
  bool infer(const Luau::MagicFunctionCallContext &context) override {
    if (context.callSite->args.size == 1)
      if (auto literal = context.callSite->args.data[0]
                             ->as<Luau::AstExprConstantString>()) {
        std::string name(literal->value.data, literal->value.size);

        if (!instance(*context.solver->rootScope, name))
          report(context, literal->location,
                 "Unknown instance class '" + name + "'");
      }
  ...
  void refine(const Luau::MagicRefinementContext &context) override {
    if (!context.callSite->self || context.callSite->args.size != 1 ||
        context.discriminantTypes.empty())
      return;

    auto literal =
        context.callSite->args.data[0]->as<Luau::AstExprConstantString>();

    if (!literal || !context.discriminantTypes[0])
      return;

    auto type = instance(*context.scope,
                         std::string(literal->value.data, literal->value.size));

    if (type)
      Luau::asMutable(*context.discriminantTypes[0])
          ->ty.emplace<Luau::BoundType>(*type);
  }
};
...
struct Lookup : Luau::MagicFunction {
  ...
  std::optional<Luau::TypeId> lookup(const Luau::Scope &scope,
                                     const Luau::AstExprCall &call,
    ...
    if (kind != LookupKind::Child) {
      ...
      auto tagged = [&](const char *tag) {
        return std::find(type->tags.begin(), type->tags.end(), tag) !=
               type->tags.end();
      };

      if ((kind == LookupKind::Service && tagged("RobloxClass") &&
           !tagged("RobloxService")) ||
    ...
    if (receiver && (kind == LookupKind::Child || kind == LookupKind::Service))
      if (auto type = Luau::get<Luau::ExternType>(Luau::follow(*receiver)))
        if (auto mapped =
                std::dynamic_pointer_cast<Instances>(type->userData)) {
          if (kind == LookupKind::Child) {
  ...
  handleOldSolver(Luau::TypeChecker &checker, const Luau::ScopePtr &scope,
    ...
    if (!result) {
      if (kind != LookupKind::Child && call.args.size &&
    ...
    if (kind == LookupKind::Class)
      result = checker.currentModule->internalTypes->addType(
          Luau::UnionType{{*result, checker.nilType}});

    return Luau::WithPredicate<Luau::TypePackId>{
        checker.currentModule->internalTypes->addTypePack({*result})};
  }

  bool infer(const Luau::MagicFunctionCallContext &context) override {
    auto result = lookup(*context.solver->rootScope, *context.callSite,
                         Luau::first(context.arguments));

    if (!result) {
      if (kind != LookupKind::Child && context.callSite->args.size &&
          context.callSite->args.data[0]->is<Luau::AstExprConstantString>())
        report(context, context.callSite->args.data[0]->location,
               "Invalid Roblox class for this operation");
    ...
    if (kind == LookupKind::Class)
      result = context.solver->arena->addType(
          Luau::UnionType{{*result, context.solver->builtinTypes->nilType}});

    Luau::asMutable(context.result)
        ->ty.emplace<Luau::BoundTypePack>(
            context.solver->arena->addTypePack({*result}));

    return true;
  }
};

void attach(Luau::TypeId type,
...
void attach(Luau::ExternType &type, const std::string &name,
            std::shared_ptr<Luau::MagicFunction> magic) {
...
Luau::TypeId external(Luau::Frontend &frontend, const std::string &name,
  ...
  return frontend.globals.globalTypes.addType(
      Luau::ExternType{name, {}, parent, {}, {}, {}, "roblox", {}});
}
} // namespace
...
std::vector<Luau::TypeId> prepareRoblox(Luau::Frontend &frontend,
  ...
  for (size_t index = 0;; ++index) {
    ...
    if (auto type =
            Luau::getMutable<Luau::ExternType>(Luau::follow(binding->type))) {
  ...
  if (instance) {
    auto type =
        Luau::getMutable<Luau::ExternType>(Luau::follow(instance->type));

    if (type) {
      attach(*type, "IsA", std::make_shared<Predicate>());

      attach(*type, "WaitForChild",
             std::make_shared<Lookup>(LookupKind::Child));

      attach(*type, "FindFirstChild",
             std::make_shared<Lookup>(LookupKind::Child));
      ...
      for (auto &binding : scope->exportedTypeBindings)
        if (auto derived = Luau::getMutable<Luau::ExternType>(
                Luau::follow(binding.second.type))) {
          auto base = derived;

          while (base != type && base && base->parent)
            base =
                Luau::getMutable<Luau::ExternType>(Luau::follow(*base->parent));
          ...
          for (const auto &candidate : scope->exportedTypeBindings)
            if (auto child = Luau::get<Luau::ExternType>(
                    Luau::follow(candidate.second.type));
                child && child->parent &&
                Luau::follow(*child->parent) ==
                    Luau::follow(binding.second.type))
              ancestor = true;
          ...
          if (property && property->readTy) {
            ...
            replacement.readTy =
                ancestor ? frontend.builtinTypes->stringType
                         : arena.addType(Luau::SingletonType{
                               Luau::StringSingleton{binding.first}});
  ...
  if (auto binding = scope->lookup(Luau::Symbol{Luau::AstName("Instance")}))
    if (auto table = Luau::get<Luau::TableType>(Luau::follow(*binding))) {
      ...
      if (constructor != table->props.end() && constructor->second.readTy)
        attach(*constructor->second.readTy,
               std::make_shared<Lookup>(LookupKind::Constructor));
    }

  if (auto model = scope->lookupType("DataModel"))
    if (auto type =
            Luau::getMutable<Luau::ExternType>(Luau::follow(model->type)))
      attach(*type, "GetService",
             std::make_shared<Lookup>(LookupKind::Service));

  for (const auto &entry :
       std::vector<std::pair<std::string, std::vector<const char *>>>{
           {"DataModel", {"game", "Game"}},
  ...
  for (size_t index = 0;; ++index) {
    ...
    Luau::getMutable<Luau::ExternType>(types.back())->userData =
        std::make_shared<Instances>();

    names.push_back(name);
    auto parent = record->substr(end + 1);

    parents.push_back(parent.empty()
                          ? std::nullopt
                          : std::optional<size_t>{std::stoull(parent)});
  }
  ...
  for (size_t index = 0; index < types.size(); ++index) {
    ...
    if (inherited) {
      ...
      if (replacement.readTy)
        replacement.readTy = parents[index] ? types.at(*parents[index])
    ...
    if (parents[index]) {
      ...
      std::static_pointer_cast<Instances>(type->userData)
          ->children[names[index]] = types[index];
  ...
  if (!types.empty() &&
      Luau::get<Luau::ExternType>(types[0])->name == "DataModel") {
    scope->bindings[Luau::AstName("game")] = Luau::Binding{types[0]};
    scope->bindings[Luau::AstName("Game")] = Luau::Binding{types[0]};

    for (size_t index = 0; index < types.size(); ++index)
      if (Luau::get<Luau::ExternType>(types[index])->name == "Workspace") {
        scope->bindings[Luau::AstName("workspace")] =
            Luau::Binding{types[index]};
