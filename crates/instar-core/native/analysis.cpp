...
extern "C" {
struct Bytes { const char* data; size_t size; };
...
struct Files : Luau::FileResolver {
    ...
    std::optional<Luau::SourceCode> readSource(const Luau::ModuleName& name) override {
        ...
        if (!source.data) return std::nullopt;
...
static Luau::ConfigOptions configurationOptions() {
    ...
    // Parse inherited files from ancestor to descendant, replacing individual aliases.
...
struct Configurations : Luau::ConfigResolver {
    ...
    const Luau::Config& getConfig(const Luau::ModuleName& name, const Luau::TypeCheckLimits&) const override {
        ...
        if (strict) result.mode = Luau::Mode::Strict;
        for (size_t index = 0;; ++index) {
            Bytes contents = configuration(context, bytes(name), index);
            if (!contents.data) break;
...
extern "C" void instar_analyze(void* context, Read read, Resolve resolve, Configuration configuration, Report report, Annotate annotate,
    const Bytes* modules, size_t count, bool strict, bool oldSolver, bool annotations) noexcept {
    // ponytail: serialize Luau's process-global state; isolated processes can provide parallel analysis.
